...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  $trendyolproduct = $_POST["trendyolproduct"];
  $pages = $_POST["pages"];
  $liste = [];
  $linkler_ve_resimler = [];

  // Trendyol'un gerçek URL'sini tanımlayın
  $base_url = "https://www.trendyol.com";

  for ($sayfa_numarasi = 1; $sayfa_numarasi <= $pages; $sayfa_numarasi++) {
      $url = $base_url . "/sr?q=" . urlencode($trendyolproduct) . "&pi=" . $sayfa_numarasi;
      $response = file_get_contents($url);
      $html_icerigi = $response;
      $html_icerigi = mb_convert_encoding($html_icerigi, 'UTF-8', 'ISO-8859-9');
      $html_icerigi = preg_replace('/[^(\x20-\x7F)]*/', '', $html_icerigi);
      $doc = new DOMDocument();
      libxml_use_internal_errors(true);
      $doc->loadHTML($html_icerigi);
      libxml_clear_errors();

      // Ürün linklerini ve resimlerini alın
      $divler = $doc->getElementsByTagName("div");
      foreach ($divler as $divElement) {
          if (strpos($divElement->getAttribute("class"), "p-card-chldrn-cntnr card-border") !== false) {
              $urunler = $divElement->getElementsByTagName("a");
              foreach ($urunler as $urunElement) {
                  $link = $urunElement->getAttribute("href");
                  if (!empty($link)) {
                      // Ürün linkini oluşturun
                      $tam_link = $base_url . $link;
                      $linkler_ve_resimler[] = array("Link" => $tam_link);
                  }
              }
          }
      }

      $isimler = $doc->getElementsByTagName("h3");
      $fiyatlar = [];

      $prcElements = $doc->getElementsByTagName("div");
      foreach ($prcElements as $element) {
          if (strpos($element->getAttribute("class"), "prc-box-dscntd") !== false) {
              $fiyatlar[] = $element;
          }
      }

      foreach ($isimler as $i => $isimElement) {
          $isim = $isimElement->textContent;
          $fiyatElement = $fiyatlar[$i];
          $fiyat = str_replace("TL", " TL", trim($fiyatElement->textContent));

          $liste[] = array("Ürün İsmi" => $isim, "Fiyat" => $fiyat);
      }
  }
}
?>
        ...
        <div class="main-panel">
          <div class="content-wrapper">
            <div class="page-header">
              <h3 class="page-title">Trendyol Product Search</h3>
              <nav aria-label="breadcrumb">
                <ol class="breadcrumb">
                  <li class="breadcrumb-item"><a href="#">Trendyol</a></li>
                ...
                <div class="card-body">
                    ...
                    <form class="forms-sample" method="POST" action="">
                        <div class="form-group">
                            ...
                            <input type="text" name="trendyolproduct" class="form-control" id="exampleInputUsername1" placeholder="Product Name">
                        </div>
                        <div class="form-group">
                    ...
                    <tbody>
                          ...
                          if (!empty($liste)) {
                              foreach ($liste as $i => $item) {
                                  ...
                                  // Ürün linkini ekleyin
                                  echo "<td><a href='" . $linkler_ve_resimler[$i]["Link"] . "' target='_blank'>Link</a></td>";
                                  echo "</tr>";
                              }
                          } else {
